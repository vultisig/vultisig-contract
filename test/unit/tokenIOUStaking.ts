...
describe("TokenIOU Staking", function () {
    ...
    async function deployFixture() {
        ...
        const tokenIOUStakingBasic = await upgrades.deployProxy(TokenIOUStakingBasic,
            [tokenIOU.target, rewardToken.target, treasury.address, 0]
        );

        const joeStaking = await upgrades.deployProxy(StableJoeStaking, [rewardToken.target, joe.address, 0],
            {
                unsafeAllow: ["constructor", "state-variable-immutable"],
                constructorArgs: [tokenIOU.target],
            });

        console.log("USDC decimals is: " + (await rewardToken.decimals()).toString());

        await tokenIOU.connect(alice).approve(tokenIOUStaking.target, parseEther("360000"));
        await tokenIOU.connect(bob).approve(tokenIOUStaking.target, parseEther("360000"));
        await tokenIOU.connect(carol).approve(tokenIOUStaking.target, parseEther("100000"));
        await tokenIOU.connect(joe).approve(tokenIOUStaking.target, parseEther("100000"));

        await tokenIOU.connect(alice).approve(tokenIOUStakingBasic.target, parseEther("360000"));
        await tokenIOU.connect(bob).approve(tokenIOUStakingBasic.target, parseEther("360000"));
        await tokenIOU.connect(carol).approve(tokenIOUStakingBasic.target, parseEther("100000"));
        await tokenIOU.connect(joe).approve(tokenIOUStakingBasic.target, parseEther("100000"));

        await tokenIOU.connect(alice).approve(joeStaking.target, parseEther("100000"));
        await tokenIOU.connect(bob).approve(joeStaking.target, parseEther("100000"));
        await tokenIOU.connect(carol).approve(joeStaking.target, parseEther("100000"));
        await tokenIOU.connect(joe).approve(joeStaking.address, parseEther("100000"));
    ...
    describe("should allow deposits and withdraws", function () {
        ...
        it("should allow deposits and withdraws of multiple users", async function () {
            ...
            expect(
                await tokenIOU.balanceOf(tokenIOUStaking.address)
            ...
            expect(await tokenIOU.balanceOf(tokenIOUStaking.address)).to.be.equal(parseEther("300"));
            expect((await tokenIOUStaking.getUserInfo(bob.address, tokenIOU.address))[0]).to.be.equal(parseEther("200"));
            ...
            // 291 + 300 * 0.97
            expect(await tokenIOU.balanceOf(tokenIOUStaking.address)
            ).to.be.equal(parseEther("600"));
            expect((await tokenIOUStaking.getUserInfo(carol.address, tokenIOU.address))[0]
            ...
            expect(await tokenIOU.balanceOf(tokenIOUStaking.address)).to.be.equal(parseEther("500"));
            expect((await tokenIOUStaking.getUserInfo(alice.address, tokenIOU.address))[0]).to.be.equal(0);
            ...
            expect(await tokenIOU.balanceOf(tokenIOUStaking.address)).to.be.equal(parseEther("400"));
            expect((await tokenIOUStaking.getUserInfo(carol.address, tokenIOU.address))[0]).to.be.equal(parseEther("200"));
            ...
            expect(await tokenIOU.balanceOf(tokenIOUStaking.address)).to.be.closeTo(
                parseEther("400"), parseEther("0.0001")
            );
            expect((await tokenIOUStaking.getUserInfo(bob.address, tokenIOU.address))[0]).to.be.closeTo(
        ...
        it("should update variables accordingly", async function () {
            ...
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("1"));

            expect(await rewardToken.balanceOf(tokenIOUStaking.address)).to.be.equal(parseEther("1"));
            expect(await tokenIOUStaking.lastRewardBalance(rewardToken.address)).to.be.equal("0");

            //increase to 7 days, as staking multiplier is 1x then.
            await increase(86400 * 7);

            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.closeTo(
                parseEther("0.5"),
                parseEther("0.0001")
            );
            ...
            expect(
                await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)
        ...
        it("should return rewards with staking multiplier accordingly", async function () {
            ...
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.closeTo(parseEther("0.5"), parseEther("0.0001"));
            ...
            // console.log("Staking multiplier is now: " + (await tokenIOUStaking.getStakingMultiplier(alice.address)).toString());
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.closeTo(parseEther("0.75"), parseEther("0.0001"));

            //increase to 1 year, as staking multiplier is 2x then.
            await increase(86400 * 185);
            // console.log("Staking multiplier is now: " + (await tokenIOUStaking.getStakingMultiplier(alice.address)).toString());
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.closeTo(parseEther("1"), parseEther("0.0001"));

            // Making sure that `pendingReward` still return the accurate tokens even after updating pools
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)
            ).to.be.closeTo(parseEther("1"), parseEther("0.0001"));
        ...
        it("should allow deposits and withdraws of multiple users and distribute rewards accordingly", async function () {
            ...
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("6"));
            // console.log("Reward pool balance: " + (await rewardToken.balanceOf(tokenIOUStaking.address)).toString());
        ...
        it("should allow deposits and withdraws of multiple users and distribute rewards accordingly even if someone enters or leaves", async function () {
            ...
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("100"));
            ...
            console.log("Reward pool balance: " + (await rewardToken.balanceOf(tokenIOUStaking.address)).toString());
            console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
            console.log("Pending reward for Alice: " + utils.formatEther((await tokenIOUStaking.pendingReward(alice.address, rewardToken.address))));
            console.log("--------------------------------------");
            console.log("Staking multiplier for Bob: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(bob.address)));
            console.log("Pending reward for Bob: " + utils.formatEther(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)));
            console.log("--------------------------------------");
            console.log("Staking multiplier for Carol: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(carol.address)));
            console.log("Pending reward for Carol: " + utils.formatEther(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address)));
            ...
            console.log("Reward pool balance: " + utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)).toString());
            console.log("Pending reward for Alice: " + utils.formatEther(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)));
            ...
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.lte(await rewardToken.balanceOf(tokenIOUStaking.address));

            console.log("Staking deposit for Alice: " + (await tokenIOUStaking.getUserInfo(alice.address, rewardToken.address))[0]);
            console.log("Staking deposit for Carol: " + (await tokenIOUStaking.getUserInfo(carol.address, rewardToken.address))[0]);
            console.log("Staking deposit for Bob: " + (await tokenIOUStaking.getUserInfo(bob.address, rewardToken.address))[0]);
            ...
            console.log("--------------------------------------");
            console.log("Reward pool balance at the end: " + utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)).toString());
            console.log("--------------------------------------");
            console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
            console.log("Staking deposit for Alice: " + (await tokenIOUStaking.getUserInfo(alice.address, rewardToken.address))[0]);
            ...
            console.log("Pending reward for Bob: " + utils.formatEther(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)));
            console.log("Reward pool balance before last withdraw: " + utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)).toString());
            ...
            expect(await rewardToken.balanceOf(tokenIOUStaking.address)).to.be.eq(parseEther("50"));
            expect(await rewardToken.balanceOf(bob.address)).to.be.closeTo(
                parseEther("100"),
                parseEther("0.001")
            );
        ...
        it("pending tokens function should return the same number of token that user actually receive", async function () {
            ...
            await rewardToken.mint(tokenIOUStaking.address, parseEther("100")); // We send 100 Tokens to sJoe's address

            await increase(86400 * 7);

            const pendingReward = await tokenIOUStaking.pendingReward(alice.address, rewardToken.address);
        ...
        it("should allow rewards in tokenIOU and USDC", async function () {
            ...
            increase(86400 * 7);
            await rewardToken.mint(tokenIOUStaking.address, parseEther("3"));
            ...
            await tokenIOUStaking.addRewardToken(tokenIOU.address);
            await tokenIOU.mint([tokenIOUStaking.address], [parseEther("6")]);
        ...
        it("should linearly increase staking multiplier after 7 days", async function () {
            ...
            await tokenIOUStaking.addRewardToken(usdc.address);
            await usdc.mint(tokenIOUStaking.address, parseEther("100"));
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("0"));
            await tokenIOUStaking.connect(alice).deposit(1);
            increase(86400 * 7);
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("50"));
            increase(86400 * 30);
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("54.335"));
            increase(86400 * 60);
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("63.005"));
            increase(86400 * 83);
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("75"));
            increase(86400 * 90);
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("87.16"));
            increase(86400 * 95);
            expect(await tokenIOUStaking.pendingReward(alice.address, usdc.address)).to.be.equal(parseEther("100"));
        });
        ...
        it("rewardDebt should be updated as expected, alice deposits before last reward is sent", async function () {
            ...
            await usdc.mint(tokenIOUStaking.address, parseEther("100"));
            console.log("USDC Staking balance: ", utils.formatEther(await usdc.balanceOf(tokenIOUStaking.address)));
            const pendingRewardBob = await tokenIOUStaking.pendingReward(bob.address, usdc.address)
            ...
            console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
            let userInfo = await tokenIOUStaking.getUserInfo(alice.address, rewardToken.address);
            console.log("Staking deposit for Alice: " + userInfo[0]);
            console.log("Pending reward for Alice: " + utils.formatEther(await tokenIOUStaking.pendingReward(alice.address, usdc.address)));
            console.log("--------------------------------------");
            console.log("Staking multiplier for Bob: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(bob.address)));
            console.log("Pending reward for Bob: " + utils.formatEther(await tokenIOUStaking.pendingReward(bob.address, usdc.address)));
            console.log("USDC Bob balance: ", utils.formatEther(await usdc.balanceOf(bob.address)));
            userInfo = await tokenIOUStaking.getUserInfo(bob.address, rewardToken.address);
            ...
            await tokenIOUStaking.removeRewardToken(usdc.address);
        });
        ...
        it("rewardDebt should be updated as expected, alice deposits after last reward is sent", async function () {
            ...
            await usdc.mint(tokenIOUStaking.address, parseEther("1"));
        ...
        it("should allow adding and removing a rewardToken, only by owner", async function () {
            ...
            await expect(
                tokenIOUStaking.connect(alice).addRewardToken(token1.address)
            ).to.be.revertedWith("OwnableUnauthorizedAccount");
            expect(
                await tokenIOUStaking.isRewardToken(token1.address)
            ).to.be.equal(false);
            ...
            await expect(
                tokenIOUStaking.connect(dev).addRewardToken(token1.address)
        ...
        it("should allow emergency withdraw", async function () {
            ...
            expect(await rewardToken.balanceOf(alice.address)).to.be.equal(0);
            expect(await tokenIOU.balanceOf(tokenIOUStaking.address)).to.be.equal(0);
            const userInfo = await tokenIOUStaking.getUserInfo(alice.address, rewardToken.address);
        ...
        it("should properly calculate and distribute rewards for multiple users in different time periods ", async function () {
            ...
            console.log("Reward pool balance: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)));
            console.log("accRewardPerShare: ", utils.formatEther(await tokenIOUStaking.accRewardPerShare(rewardToken.address)));
            ...
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.equal(parseEther("0"));
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(parseEther("0"));
            expect(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address)).to.be.equal(parseEther("0"));
            ...
            console.log("Reward balance after all withdrawals: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)));
        ...
        it.skip("should calculate rewards correctly when the number of depositors is >= 200", async function () {
            ...
            for (let i = 0; i < 200; i++) {
                ...
                await tokenIOU.connect(signer).approve(tokenIOUStaking.address, parseEther("1000"));
        ...
        it("pending reward should be updated for all stakers when there is a new deposit of reward tokens", async function () {
            ...
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.equal(parseEther("33.333333333333333333"));
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(parseEther("66.666666666666666666"));
        ...
        it("extra rewards should be distributed to community plus stakers", async function () {
            ...
            console.log("Pending reward for Bob: " + utils.formatEther(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)));
            console.log("--------------------------------------");
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(parseEther("25"));

            await tokenIOUStaking.connect(alice).withdraw(parseEther("50")); // unclaimed rewards so far = 25
            expect(await tokenIOUStaking.forgoneRewardsPool(rewardToken.address)).to.be.equal(parseEther("25"));
            ...
            console.log("Reward pool balance: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)));
            console.log("Total staked balance: ", utils.formatEther(await tokenIOU.balanceOf(tokenIOUStaking.address)));

            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(
                parseEther("25")
            );
            ...
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.equal(parseEther("16.666666666666666666"));
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(parseEther("34.511666666666666666"));

            await tokenIOUStaking.connect(alice).withdraw(parseEther("50"));
            expect(await tokenIOUStaking.forgoneRewardsPool(rewardToken.address)).to.be.closeTo(parseEther("41.666"), parseEther("0.001"));
            ...
            expect(await rewardToken.balanceOf(tokenIOUStaking.address)).to.be.closeTo(parseEther("41.6666"), parseEther("0.0001"));
            ...
            increase(86400 * 365);
            userInfo = await tokenIOUStaking.getUserInfo(alice.address, rewardToken.address);
            console.log("Staking deposit for Alice: " + utils.formatEther(userInfo[0]));
            console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
        ...
        it("claimExtraRewards should not underflow", async function () {
            ...
            await tokenIOUStaking.connect(alice).withdraw(parseEther("50"));
            expect(await tokenIOUStaking.forgoneRewardsPool(rewardToken.address)).to.be.equal(parseEther("41.666666666666666667"));
            ...
            // Extra rewards claim redistribution
            await expect(tokenIOUStaking.connect(alice).withdrawAndClaimExtraRewards(0)).to.be.revertedWith("tokenIOUStaking: not eligible for extra rewards");
            expect(await tokenIOUStaking.connect(alice).pendingExtraRewards(alice.address, rewardToken.address)).to.be.equal(0);
            let userInfo = await tokenIOUStaking.getUserInfo(alice.address, rewardToken.address);
            console.log("Staking deposit for Alice: " + utils.formatEther(userInfo[0]));
            ...
            console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
            console.log("Reward balance before Alice: ", utils.formatEther(await rewardToken.balanceOf(alice.address)));
            expect(await tokenIOUStaking.connect(alice).pendingExtraRewards(alice.address, rewardToken.address)).to.be.closeTo(parseEther("20.83"), parseEther("0.001"));
        ...
        it.skip("unclaimed rewards should be redistributed to other stakers", async function () {
            ...
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.equal(parseEther("0"));
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(parseEther("50"));
            ...
            expect(await rewardToken.balanceOf(tokenIOUStaking.address)).to.be.closeTo(parseEther("0"), parseEther("0.00001"));
        });
        ...
        it("redistribution of rewards after an early withdrawal", async function () {
            ...
            await tokenIOUStaking.connect(bob).deposit(parseEther("100"));
            await increase(86400 * 7);
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("200"));
            ...
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("3"));

            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.equal(parseEther("51"));
            expect(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address)).to.be.equal(parseEther("52"));
        ...
        it("Pending rewards can't exceed the reward pool when remittance is sent after large depositors", async function () {
            ...
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("10"));
            await increase(86400 * 10);
            console.log("Reward pool balance: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)));
            ...
            // console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
            console.log("Pending reward for Alice: " + utils.formatUnits(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address), 18));
            console.log("--------------------------------------");
            // console.log("Staking multiplier for Bob: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(bob.address)));
            console.log("Pending reward for Bob: " + utils.formatUnits(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address), 18));
            console.log("--------------------------------------");
            // console.log("Staking multiplier for Carol: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(carol.address)));
            console.log("Pending reward for Carol: " + utils.formatUnits(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address), 18));

            //Total pending reward amount can't exceed the reward pool balance
            expect((await tokenIOUStaking.pendingReward(alice.address, rewardToken.address))
                .add(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address))
                .add(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address))
            ).to.be.lte(await rewardToken.balanceOf(tokenIOUStaking.address));
        ...
        it("Pending rewards can't exceed the reward pool in realistic scenario", async function () {
            ...
            await tokenIOUStaking.connect(bob).deposit(parseEther("200"));
            await tokenIOUStaking.connect(carol).deposit(parseEther("300"));
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("300"));
        ...
        it("Special case logic exploit can't exceed reward pool balance", async function () {
            ...
            console.log("--------------------------------------");
            userInfo = await tokenIOUStaking.getUserInfo(carol.address, rewardToken.address);
        ...
        it("Staking rewards redistribution to community plus users", async function () {
            ...
            await rewardToken.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("1000"));
            ...
            console.log("Pending reward for Alice: " + utils.formatEther((await tokenIOUStaking.pendingReward(alice.address, rewardToken.address))));
            console.log("--------------------------------------");
            expect(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address)).to.be.equal(parseEther("200"));
            ...
            console.log("Staking multiplier for Carol: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(carol.address)));
            console.log("Pending reward for Carol: " + utils.formatEther(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address)));
            expect(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address)).to.be.equal(parseEther("500"));
            await tokenIOUStaking.connect(carol).withdraw(parseEther("0"));
        ...
        it("Production test case simulation, ensures no funds end up unclaimable", async function () {
            ...
            await tokenIOU.connect(tokenIOUMaker).transfer(tokenIOUStaking.address, parseEther("100"));
            await tokenIOUStaking.addRewardToken(tokenIOU.address);
            ...
            console.log("Forgone USDC reward pool balance: " + utils.formatEther(await tokenIOUStaking.forgoneRewardsPool(rewardToken.address)));
            console.log("Forgone tokenIOU reward pool balance: " + utils.formatEther(await tokenIOUStaking.forgoneRewardsPool(tokenIOU.address)));

            console.log("Reward USDC pool balance: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStaking.address)));
            console.log("Total tokenIOU pool balance: ", utils.formatEther(await tokenIOU.balanceOf(tokenIOUStaking.address)));
            console.log("--------------------------------------");
            console.log("Staking multiplier for Alice: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(alice.address)));
            console.log("Pending USDC reward for Alice: " + utils.formatUnits(await tokenIOUStaking.pendingReward(alice.address, rewardToken.address), 18));
            console.log("Pending tokenIOU reward for Alice: " + utils.formatUnits(await tokenIOUStaking.pendingReward(alice.address, tokenIOU.address), 18));
            console.log("--------------------------------------");
            console.log("Staking multiplier for Bob: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(bob.address)));
            console.log("Pending USDC reward for Bob: " + utils.formatUnits(await tokenIOUStaking.pendingReward(bob.address, rewardToken.address), 18));
            console.log("Pending tokenIOU reward for Bob: " + utils.formatUnits(await tokenIOUStaking.pendingReward(bob.address, tokenIOU.address), 18));
            console.log("--------------------------------------");
            console.log("Staking multiplier for Carol: " + utils.formatEther(await tokenIOUStaking.getStakingMultiplier(carol.address)));
            console.log("Pending USDC reward for Carol: " + utils.formatUnits(await tokenIOUStaking.pendingReward(carol.address, rewardToken.address), 18));
            console.log("Pending tokenIOU reward for Carol: " + utils.formatUnits(await tokenIOUStaking.pendingReward(carol.address, tokenIOU.address), 18));

            console.log("- - - END -- STATE- -- - - -- -");

            //Forgone rewards from Bob should equal 125 tokenIOU
            expect(await tokenIOUStaking.forgoneRewardsPool(tokenIOU.address)).to.be.equal(parseEther("125"));
        ...
        it("Ensure forgoneRewardsPool never goes over the available amount of tokens", async function () {
            ...
            expect(await tokenIOUStaking.forgoneRewardsPool(rewardToken.address)).to.be.closeTo(parseEther("116.693"), parseEther("0.001"));
        ...
        it("Auto staking", async function () {
            ...
            console.log("Reward USDC pool balance: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStakingBasic.address)));
            console.log("Before Total tokenIOU pool balance: ", utils.formatEther(await tokenIOU.balanceOf(tokenIOUStakingBasic.address)));
            console.log("--------------------------------------");
            console.log("Pending USDC reward for Alice: " + utils.formatUnits(await tokenIOUStakingBasic.pendingReward(alice.address, rewardToken.address), 18));
            console.log("Pending USDC reward for Bob: " + utils.formatUnits(await tokenIOUStakingBasic.pendingReward(bob.address, rewardToken.address), 18));
            ...
            console.log("Reward USDC pool balance: ", utils.formatEther(await rewardToken.balanceOf(tokenIOUStakingBasic.address)));
            console.log("After Total tokenIOU pool balance: ", utils.formatEther(await tokenIOU.balanceOf(tokenIOUStakingBasic.address)));
            console.log("--------------------------------------");
            console.log("Pending USDC reward for Alice: " + utils.formatUnits(await tokenIOUStakingBasic.pendingReward(alice.address, rewardToken.address), 18));
            console.log("Pending USDC reward for Bob: " + utils.formatUnits(await tokenIOUStakingBasic.pendingReward(bob.address, rewardToken.address), 18));
