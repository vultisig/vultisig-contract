...
describe("Merge Contract", function () {
  ...
  describe("onApprovalReceived Function", function () {
    it("Should revert if called by a non-VULT token", async function () {
      ...
      await expect(mockVult["approveAndCall(address,uint256)"](merge, 100)).to.be.revertedWithCustomError(
        merge,
        "InvalidTokenReceived",
      );
    });
    ...
    it("Should revert if locked status is not TwoWay", async function () {
      ...
      await expect(vult["approveAndCall(address,uint256)"](merge, 100)).to.be.revertedWithCustomError(
        merge,
        "VultToWeweNotAllwed",
      );
      await merge.setLockedStatus(1);
    ...
    it("Should revert if amount is zero", async function () {
      ...
      await expect(vult["approveAndCall(address,uint256)"](merge, 0)).to.be.revertedWithCustomError(
        merge,
        "ZeroAmount",
      );
    });
    ...
    it("Should correctly transfer VULT and WEWE", async function () {
      ...
      await expect(
        vult.connect(otherAccount)["approveAndCall(address,uint256)"](merge, vultDeposit),
      ).to.be.revertedWithCustomError(vult, "TransferLocked");
      await vult.setLocked(false);
      await vult.connect(otherAccount)["approveAndCall(address,uint256)"](merge, vultDeposit),
